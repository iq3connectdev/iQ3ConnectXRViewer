...
**Face tracking data**: iQ3Connect XR Viewer exposes AR Kit Face Tracking data to enable in-browser AR experience like avatars with animated faces or the addition of special effects on top of video conferencing feeds (e.g. virtual glasses, hats, etc). Face tracking data is not stored locally, only used to enable accurate face tracking, apply visual effects, and deliver interactive augmented reality experiences. All data is processed in real time on the user’s device, is never stored or transmitted, and is not used to identify or profile the user in any way. Users are prompted when a website tries to access the camera.

**Depth API and scene geometry data**: Depth and Scene geometry data is used to make interaction with real-world objects look more realistic in AR. This includes being able to place objects naturally on surfaces as well as having realistic occlusions. iQ3Connect XR Viewer does not locally store depth or geometry data. All data is processed in real time on the user’s device, and is never stored or transmitted. All websites that request this data will prompt a user with a confirmation dialog.
