extension ARKController {
    ...
    func updateARKData(with frame: ARFrame) {
        ...
        synced(self) {
            ...
            var blockTimes: [String: TimeInterval] = [:]
            ...
            if (request[WEB_AR_LIGHT_INTENSITY_OPTION] as? NSNumber)?.boolValue ?? false {
                let lightBlockStart = CFAbsoluteTimeGetCurrent()
                // Light intensity block code...
                ...
                blockTimes["WEB_AR_LIGHT_OBJECT_OPTION"] = CFAbsoluteTimeGetCurrent() - lightBlockStart
            }
            
            if (request[WEB_AR_CAMERA_OPTION] as? NSNumber)?.boolValue ?? false {
                let cameraBlockStart = CFAbsoluteTimeGetCurrent()
                // Camera block code...
                ...
                blockTimes["WEB_AR_CAMERA_OPTION"] = CFAbsoluteTimeGetCurrent() - cameraBlockStart
            }
            
            if (request[WEB_AR_3D_OBJECTS_OPTION] as? NSNumber)?.boolValue ?? false {
                let objectsBlockStart = CFAbsoluteTimeGetCurrent()
                // 3D objects block code...
                ...
                blockTimes["WEB_AR_3D_OBJECTS_OPTION"] = CFAbsoluteTimeGetCurrent() - objectsBlockStart
            }
            if computerVisionDataEnabled && computerVisionFrameRequested {
                let cvBlockStart = CFAbsoluteTimeGetCurrent()
                ...
                blockTimes["computer_vision"] = CFAbsoluteTimeGetCurrent() - cvBlockStart
            }
            
            // Log execution times
            let totalTime = CFAbsoluteTimeGetCurrent() - startTime
            print("ARKController updateARKData execution times (ms):")
            print("Total time: \(totalTime * 1000)")
            for (block, time) in blockTimes {
                print("\(block) block: \(time * 1000)")
            }
