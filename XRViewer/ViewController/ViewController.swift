...
class ViewController: UIViewController, UIGestureRecognizerDelegate { /// GCDWebServer change
#if WEBSERVER
//    private var webServer: GCDWebServer?
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.bool(forKey: Constant.permissionsUIAlreadyShownKey()) == false && (locationManager.authorizationStatus == .notDetermined || AVCaptureDevice.authorizationStatus(for: .video) == .notDetermined) {
            DispatchQueue.main.async(execute: {
...
//                let rightSafeAreaInset = UIApplication.shared.keyWindow?.safeAreaInsets.right ?? 0.0
                let rightSafeAreaInset = view.safeAreaInsets.right;
...
//                let leftSafeAreaInset = CGFloat(UIApplication.shared.keyWindow?.safeAreaInsets.left ?? 0.0)
                let leftSafeAreaInset = view.safeAreaInsets.left;
