...
//        }
        ...
        } else if theUrl?.hasPrefix("http://") ?? false || theUrl?.hasPrefix("https://") ?? false {
            url = URL(string: theUrl ?? "")
        } else {
            url = URL(string: "https://\(theUrl ?? "")")
        }
...
//        }
        ...
        if homeURL != nil && homeURL != "" {
            loadURL(homeURL)
        } else if let homepageContent = Bundle.main.url(forResource: "homepage", withExtension: "html") {
            let request = URLRequest(url: homepageContent)
            webView?.load(request)
        } else {
            DDLogError("Homepage HTML file not found in bundle")
            loadURL(WEB_URL)
        }
    }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        } else if message.name == LOAD_WEBPAGE_CONTENT_FROM_URL {
...
//        conf.preferences = pref
        ...
        // Use WKWebpagePreferences instead of WKPreferences
        let webPref = WKWebpagePreferences()
        webPref.allowsContentJavaScript = true // Replaces deprecated javaScriptEnabled

        // Set private preferences (Use with caution)
//        pref.setValue(true, forKey: "allowFileAccessFromFileURLs")
//        pref.setValue(true, forKey: "webGLEnabled")

        pref.setValue(true, forKey: "acceleratedCompositingEnabled")
        // Optional: Enable hardware acceleration
//        webPref.setValue(true, forKey: "accelerated2dCanvasEnabled")
        
        pref.javaScriptCanOpenWindowsAutomatically = true
        pref.setValue(true, forKey: "developerExtrasEnabled")

        conf.preferences = pref
        conf.defaultWebpagePreferences = webPref
